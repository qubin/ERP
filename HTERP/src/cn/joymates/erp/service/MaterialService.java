...
public class MaterialService  extends BaseService<Material> {
	...
	public List<Map<String, Object>> find(Material material,String material_key,String material_name,String ec_rd,HttpServletRequest req) {
		...
		searchsql = "SELECT COUNT(*) FROM t_material AS m LEFT JOIN t_supply_mat AS sm ON m.supplymat_id=sm.id LEFT JOIN t_supplier AS s ON sm.supply_id = s.id LEFT JOIN t_warehouse AS w ON m.warehouse_id = w.id WHERE 1 = 1 ";
		resultsql = "SELECT m.*,s.name AS supplierName,w.sign1 AS warehouseSign FROM t_material AS m LEFT JOIN t_supply_mat AS sm ON m.supplymat_id=sm.id LEFT JOIN t_supplier AS s ON sm.supply_id = s.id LEFT JOIN t_warehouse AS w ON m.warehouse_id = w.id WHERE 1 = 1 ";
